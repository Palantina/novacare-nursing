...
const navItems: NavItem[] = [
  ...
  },
  {
    ...
    isActive: (path) => path === "/support-at-home",
  },
  {
    href: "/staying-at-home-care",
    label: "Care at Home",
    isActive: (path) =>
      path === "/staying-at-home-care" ||
      path === "/live-in-care" ||
      path === "/24-hour-care",
  },
  {
      ...
      <div className="bg-[#F2EEFF] text-[#1D3952]">
        <div className="mx-auto flex min-h-[44px] max-w-7xl items-center justify-between gap-4 px-4 py-2 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 text-sm font-semibold sm:text-base">
            <svg aria-hidden="true" className="h-5 w-5 shrink-0 text-[#6E3BE8]" viewBox="0 0 24 24" fill="none">
          ...
          {canCall ? (
            <a href="tel:+61491303671" className="flex shrink-0 items-center gap-2 text-sm font-semibold transition-colors hover:text-[#5274D8] sm:text-base" aria-label="Call NovaCare Nursing on 0491 303 671">
      ...
      <div className="border-b border-[#E4EEF1] bg-white/95 shadow-sm backdrop-blur">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex min-h-[92px] items-center justify-between gap-4">
            <Link href="/" className="flex min-w-0 items-center" aria-label="NovaCare Nursing home">
              <Image src="/NovaCare_Logo_Light.svg" alt="NovaCare Nursing" width={720} height={200} priority className="h-auto w-[210px] sm:w-[235px] lg:w-[235px]" />
            </Link>
            ...
            <button type="button" className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-[#D8E7EB] bg-white text-[#1D3952] transition hover:bg-[#EFFAFC] focus:outline-none focus:ring-4 focus:ring-[#2FBFC4]/20 lg:hidden" aria-label={menuOpen ? "Close navigation menu" : "Open navigation menu"} aria-expanded={menuOpen} aria-controls="mobile-navigation" onClick={() => setMenuOpen((current) => !current)}>
              {menuOpen ? (
                <svg aria-hidden="true" className="h-6 w-6" viewBox="0 0 24 24" fill="none"><path d="m6 6 12 12M18 6 6 18" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg>
              ) : (
                <svg aria-hidden="true" className="h-6 w-6" viewBox="0 0 24 24" fill="none"><path d="M4 7h16M4 12h16M4 17h16" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg>
              )}
            </button>
          </div>
            ...
            <div id="mobile-navigation" className="border-t border-[#E4EEF1] pb-5 pt-3 lg:hidden">
              <nav className="flex flex-col gap-1" aria-label="Mobile navigation">
                {navItems.map((item) => {
                  const active = item.isActive(pathname);
                  return (
                    <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={`rounded-xl px-4 py-3 text-base font-medium transition ${active ? "bg-[#EFFAFC] text-[#1D3952]" : "text-[#445E72] hover:bg-[#F5FAFC] hover:text-[#1D3952]"}`}>
                      {item.label}
                    </Link>
                  );
                })}
                <Link href="/contact" className="mt-2 inline-flex min-h-[50px] items-center justify-center rounded-2xl bg-[#2FBFC4] px-5 py-3 text-base font-semibold text-[#173B5C]">Contact us</Link>
              </nav>

              <div className="mt-4 border-t border-[#E4EEF1] px-4 pt-4 text-sm leading-6 text-[#445E72]">
                <p className="font-semibold text-[#1D3952]">Hobart • Huon Valley • Derwent Valley • Surrounds</p>
